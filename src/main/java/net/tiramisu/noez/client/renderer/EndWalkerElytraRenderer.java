...
public class EndWalkerElytraRenderer<E extends LivingEntity, M extends EntityModel<E>> extends ElytraLayer<E, M> {
    private static final ResourceLocation ECHO_ELYTRA_TEXTURE = new ResourceLocation(NOEZ.MOD_ID, "textures/entity/end_walker_elytra.png");
    private final ElytraModel<E> model;

    public EndWalkerElytraRenderer(RenderLayerParent<E, M> pRenderer, EntityModelSet pModelSet) {
    ...
    public void render(PoseStack matrixStack, MultiBufferSource buffer, int packedLight, E livingEntity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (itemStack.getItem() == NoezItems.END_WALKER_ELYTRA.get()) {
